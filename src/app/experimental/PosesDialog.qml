...
Dialog {
    ...
    property bool canEditPoses: root.poseBridge.rowCount > 0 && root.studyBridge.primaryModelIndex >= 0 && !root.optimizerBridge.running
    ...
    FileDialog {
        ...
        nameFilters: ["JTA Pose File (*.jtap)", "JointTrack Pose File (*.jtp)", "Pose File (*.txt)"]
    ...
    FileDialog {
        ...
        nameFilters: ["JTA Kinematics File (*.jtak)", "JointTrack Kinematics File (*.jts)", "Kinematics File (*.txt)"]
    ...
    contentItem: ColumnLayout {
        ...
        RowLayout {
            ...
            Label {
                text: root.studyBridge.primaryModelIndex >= 0 ? qsTr("Model %1 · %2 frames").arg(root.studyBridge.primaryModelIndex).arg(root.poseBridge.rowCount) : qsTr("%1 frames").arg(root.poseBridge.rowCount)
                color: Theme.fg
                font.bold: true
            ...
            Rectangle {
                ...
                radius: 7
                color: root.poseBridge.dirty ? Theme.badgeDirtyBg : Theme.badgeCleanBg

                Label {
                    id: dirtyLabel

                    anchors.centerIn: parent
                    text: root.poseBridge.dirty ? qsTr("● unsaved") : qsTr("saved")
                    color: root.poseBridge.dirty ? Theme.badgeDirtyFg : Theme.badgeCleanFg
