...
Rectangle {
    ...
    function focusSettingsOpener() {
        settingsOpenButton.forceActiveFocus();
    }

    function focusPosesOpener() {
        posesOpenButton.forceActiveFocus();
    }
    ...
    function pickImages() {
        const files = root.fileDialogBridge.getOpenFileNames(qsTr("Load Images"), "Image Files (*.tif *.tiff *.png *.TIF *.TIFF *.PNG)", "", "images");
        if (files.length === 0)
            return ;
 // A second image set is a new study: confirm, then replace the
    ...
    function pickModels() {
        const files = root.fileDialogBridge.getOpenFileNames(qsTr("Load Implant Models"), "CAD File (*.stl *.STL)", "", "models");
        if (files.length === 0)
            return ;
    ...
    RowLayout {
        ...
        RowLayout {
            ...
            Button {
                ...
                enabled: !root.studyBridge.hasCalibration && !root.runLocked
            ...
            Button {
                ...
                onClicked: {
                    if (!root.studyBridge.hasCalibration)
                        root.showMessageRequested(qsTr("Error!"), qsTr("Load Calibration First!"));
                    else
                        pickImages();
                }
            }
        ...
        Label {
            text: root.studyBridge.hasCalibration ? (root.studyBridge.calibratedForBiplane ? qsTr("Calibrated (biplane)") : qsTr("Calibrated (monoplane)")) : qsTr("No calibration")
            color: root.studyBridge.hasCalibration ? Theme.ok : Theme.fgMuted
        ...
        Rectangle {
            ...
            Layout.preferredWidth: Math.max(shellDirtyLabel.implicitWidth + 12, 28)
            radius: 7
