...
ColumnLayout {
    ...
    Label {
        Layout.fillWidth: true
        Layout.topMargin: Theme.spacingSm
        visible: root.poseBridge.rowCount > 0 && root.studyBridge.primaryModelIndex < 0
        horizontalAlignment: Text.AlignHCenter
        color: Theme.fgDim
        wrapMode: Text.Wrap
        text: qsTr("Select a model in the model list to edit its " + "poses (v1: pose ops edit the primary model).")
    }
